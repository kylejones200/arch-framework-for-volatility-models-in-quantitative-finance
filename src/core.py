...
def simulate_returns_with_volatility_clustering(n: int=1000, omega: float=0.1, alpha: float=0.8, seed: int=42) -> tuple[np.ndarray, np.ndarray]:
...
def plot_returns_volatility(returns: np.ndarray, volatility: np.ndarray, output_path: Path, plot: bool=False):
...
def plot_volatility_forecast(forecast_variance: pd.Series, output_path: Path, plot: bool=False):
    """Plot forecasted volatility"""
    if not plot:
        return
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(forecast_variance.values, marker='o', color='#4A90A4', linewidth=1.2, markersize=4)
